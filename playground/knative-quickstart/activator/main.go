...
func main() {
	...
	logger.Info("Starting the knative elasti-activator")

	// Create the transport used by both the elasti-activator->QP probe and the proxy.
	...
	// At this moment elasti-activator with TLS does not disable HTTP.
	...
	ah = handler.NewTimeoutHandler(ah, "elasti-activator request timeout", func(r *http.Request) (time.Duration, time.Duration, time.Duration) {
